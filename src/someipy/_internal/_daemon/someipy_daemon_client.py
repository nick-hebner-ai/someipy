    ...
    async def _handle_message(self, message: BaseMessage):
        ...
        if message["type"] == InboundCallMethodRequest.__name__:
            ...
            for service_instance in self._client_service_instances:
                ...
                # the caller's raw declared endpoint_port (0 when a client
                # wants the OS to pick a free port) and is never updated with
                # the real OS-assigned port, while the daemon reports the
                # real bound port here -- so this comparison could never
                # match and silently dropped every response before it
                # reached _method_call_data_received. That method already
                # does its own robust correlation via
