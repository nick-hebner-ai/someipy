...
class SomeipDaemon:
    ...
    async def _handle_outbound_call_method_request(
        ...
        new_call = MethodCall(
            ...
            # declared endpoint_port (0 for consumed services, per
            # tcu_sim's own test_someip_client.py:328) -- the response-side
            # correlation key in _someip_message_callback is built from the
            # endpoint's real OS-assigned port, so using the raw value here
            # means the two keys can never match.
